...
export default function CardItem({ props }) {
  ...
  function fetchUser() {
    axios.get(host_url + '/user', {
      params: {
        id: props.writer_id
      }
    }).then((res) => {
      var user = res.data;
      if (!user.avatar.endsWith('.jpg') && !user.avatar.endsWith('.png') && !user.avatar.endsWith('.jpeg')) {
  ...
  function fetchReactions() {
    axios.get(host_url + "/reaction/fetch?document=" + props.id).then((res) => {
      var l = 0;
      var d = 0;
  ...
  async function handleReaction(event, reaction) {
    ...
    await axios
      .get(
        host_url +
        `/reaction?document=${props.id}&user=${decodedToken.user}&reaction=${reaction ? 1 : -1
        }`
      )
      .then((res) => {
        console.log(res);
  ...
  const initFileDisplay = () => {
    ...
    for (let i = 0; i < props.file.length; i++) {
      if (!props.file[i].endsWith('.jpg') && !props.file[i].endsWith('.png') && !props.file[i].endsWith('.jpeg')) {
          ...
          <div className="whitespace-pre-wrap mt-1 font-bold">
            {props.category
              ? props.category.map((tag) => {
                const res = props.categories.find(({ id }) => id === tag) || ""
